...
namespace Controls.DragDropControl
{
    public class DragDropHelper
    {
            ...
            if (isDragging)
            {
                ...
                if (draggedItem != null)
                {
                    if (sender is ListBox)
                    {
                        ...
                        int temp = insertionIndex;
                        ScrollViewer scroll = GetScrollViewer(lb);
                        if (scroll.VerticalOffset == temp)
                        {
                            temp -= 1;
                        }

                        if (temp >= 0 && temp <= lb.Items.Count - 1 && temp > -1)
                        {
	    ...
	    private ScrollViewer GetScrollViewer(ListBox box)
	    {
            var scrollBorder = VisualTreeHelper.GetChild(box, 0);
            if (scrollBorder is Decorator) {
                var scroll = ((Decorator)scrollBorder).Child;
                if (scroll is ScrollViewer) {
                    return scroll as ScrollViewer;
                }
            }
            return null;
	    }
        ...
        private void ShowDraggedAdorner(Point currentPosition)
        {
            ...
            {
                var adornerLayer = AdornerLayer.GetAdornerLayer((UIElement)this.topWindow.Content);
                this.draggedAdorner = new DraggedAdorner(this.draggedData, GetDragDropTemplate(this.sourceItemsControl), (UIElement)this.topWindow.Content, adornerLayer);
            }
            this.draggedAdorner.SetPosition(currentPosition.X - this.mouseDelta.X, currentPosition.Y - mouseDelta.Y);
        }
